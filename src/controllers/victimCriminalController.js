    ...
    try {
        ...
        let query = `
            SELECT 
                ...
                cr.first_name as criminal_first_name,
                cr.last_name as criminal_last_name,
                cr.id_number as criminal_id_number,
        ...
        result.rows.forEach(row => {
            ...
            if (row.criminal_record_id) {
                victimsMap.get(row.vic_id).criminal_records.push({
                    ...
                    first_name: row.criminal_first_name,
                    last_name: row.criminal_last_name,
                    id_number: row.criminal_id_number,
...
const getVictimWithCriminalRecords = async (req, res) => {
    try {
        ...
        const victim = {
            ...
            id_number: result.rows[0].victim_id_number,
            date_committed: result.rows[0].victim_date_committed,
